...
### 정보허브 실데이터 전면 확장 · /programs 500 에러 수정 · 이용 가이드 · 요금제 확정

**배경:** 배포된 사이트를 사용자가 직접 확인하면서 나온 문제(제목/링크 불일치, 500 에러, 없는 페이지)를 그때그때 조사해서 고친 하루. 코드를 새로 짜기보다 "실제로 뭐가 잘못 연결돼 있는지" 찾아내는 작업이 많았다.

---

### 1. Startup News RSS 실데이터 연동 + Recommended Articles 실제 큐레이션

**커밋:** `c22ada9`

정부지원사업 소식과 같은 `hub_items` 파이프라인에 플래텀·벤처스퀘어·바이라인네트워크 RSS를 `source_type="news"`로 수집하는 `collect_startup_news()`를 추가했다. 기존 `collect_gov_support`와 거의 동일했던 upsert 로직은 `_collect()` 공용 함수로 추출해 중복을 없앴다.

Recommended Articles는 접근이 달랐다 — 이건 "이번 주 최신 글"이 아니라 "단계별로 계속 유효한 좋은 글"이 필요해서 애초에 RSS/자동 수집 대상이 될 수 없다. 대신 7개 실제 글(린 스타트업, 블루오션 전략, TAM·SAM·SOM, LTV/CAC, 런웨이 계산법, 팀빌딩, IR 피치덱)을 직접 검색해서 제목·저자·URL을 라이브로 검증한 뒤 큐레이션했다 — 이전엔 브런치 홈페이지 링크에 지어낸 제목을 붙여둔 상태였다.

---

### 2. Startup News/정부지원사업 fallback 목업, 실제 데이터로 교체

**커밋:** `98151b7`, `68dfe84`, `3f32683`

실데이터를 연동했는데도 "여전히 상세 기사로 안 들어가진다"는 지적이 계속 나와서 다시 조사했다. **GitHub Actions의 hub_items 수집 크론이 STEP9 배포(07-30) 이후 지금까지 3일 연속 전부 실패**하고 있었다 — 즉 오늘 작업 때문에 생긴 문제가 아니라 애초부터 한 번도 성공한 적이 없던 것. 그래서 화면은 계속 fallback 목업을 보여주고 있었는데, 그 목업 자체가 (Recommended Articles를 고치기 전처럼) 지어낸 헤드라인 + 홈페이지 링크였다는 게 진짜 원인이었다.

- Startup News 목업을 실제 존재하는 기사 7개로 교체, 이후 사용자가 직접 골라 검수한 기사 4건(에니그마 시드 유치, 사업계획서~투자검토, 성장전략, 충청권 IR캠프)으로 다시 갱신.
- 정부지원사업 소식 목업도 기업마당·KOCCA를 라이브로 재조회해 7/20 이후 실제 공고 7건으로 교체.
- 크론 실패의 근본 원인(GitHub/Render 시크릿 불일치로 추정)은 시크릿 값을 읽을 권한이 없어 사용자 확인이 필요한 상태로 남겨뒀다.

---

### 3. 지원사업 8건 추가 + 마감 현황 점검

**커밋:** `12d255e`

사용자가 실제로 찾은 기업마당·KOCCA·GOKAMS 공고 8건(콘텐츠코리아 전시회, 사회혁신 소셜벤처 리그, 청년창업제품 판로개척, 예술분야 예비창업 프로그램, 김해스타트업 창업캠프, 시흥 스타트업 통합 IR데이, 콘텐츠IP 마켓, 예술산업보증 공모)의 링크·설명을 받아 라이브 URL 검증 후 `SUPPORT_PROGRAMS`에 추가했다. 이 과정에서 기존 19건 중 **18건이 이미 마감**(2026-08-02 기준) 상태였다는 것도 발견 — 사실상 TIPS 1건만 살아있었는데, 이번 추가로 활성 지원사업이 9건이 됐다.

---

### 4. `/programs` 맞춤 추천 500 에러 수정

**커밋:** `c381b67`

"맞춤 지원사업 추천받기"를 누르면 500 에러가 난다는 신고를 받고 조사한 결과, `/programs/recommend`가 **프로젝트 최초 스캐폴딩 때 만든 완전히 별개의 시스템**(LangChain + FAISS 로컬 벡터 인덱스 + OpenAI 임베딩)을 호출하고 있었다. Render는 배포마다 컨테이너가 새로 뜨는 환경이라 로컬 FAISS 인덱스가 매번 사라져 재빌드를 시도하다 죽고 있었고, 그 RAG가 참조하던 데이터(`sample_programs.json`)도 지금 실제로 관리 중인 `SUPPORT_PROGRAMS`와는 아예 다른 옛날 샘플 데이터였다.

프론트에서 백엔드 호출 자체를 걷어내고, 이미 검증해둔 `SUPPORT_PROGRAMS`를 관심분야(정확 일치)·지역(느슨한 부분일치) 기준으로 클라이언트에서 즉시 필터링·정렬하도록 교체했다. 이제 안 쓰는 백엔드 RAG 관련 파일(`backend/app/api/programs.py`, `rag/pipeline.py` 등)은 삭제하지 않고 남겨뒀다.

---

### 5. `/programs` 검색에 STEP 매칭 추가 + 매칭 카드만 테두리 강조

**커밋:** `e886a9f`

검색 조건을 수정해도 결과가 안 바뀌는 것처럼 보인다는 피드백 — 실제로는 재검색 자체는 되고 있었는데, 결과 화면이 "조건에 안 맞는 건 사라지는" 방식이라 변화가 잘 안 보였던 것. 로드맵 STEP(1~7) 선택 필드를 추가해 `SUPPORT_PROGRAMS.steps`와 직접 매칭하고, 결과를 필터링해서 숨기지 않고 **활성 지원사업 전체를 항상 보여주면서** 조건(STEP·관심분야·지역)에 맞는 카드에만 보라색 테두리 + "맞춤" 배지 + 매칭 이유를 표시하는 방식으로 바꿨다.

---

### 6. 실제 화면 캡처 기반 "이용 가이드" 페이지 신설

**커밋:** `b7add9b`, `7ff4245`, `81ca9e7`, `fa91c39`

랜딩 페이지 상단 메뉴(작동 방식/지원사업 매칭/작성 화면)가 실제로는 랜딩 안의 마케팅용 스크롤 섹션(가짜 데이터 목업)으로 연결되고 있던 것을, 실제 화면을 캡처해서 "어디를 채우고 어떤 버튼을 누르면 뭐가 나오는지" 알려주는 신규 `/guide` 페이지로 바꿨다. 대시보드·로드맵 STEP(생성 전/후, 채점·피드백)·`/programs` 검색폼·매칭 결과·사업계획서 보기/편집, 총 7장의 실제 스크린샷을 찍어 단계별 설명과 함께 배치했다. 디자인은 랜딩과 동일한 토큰·컴포넌트(글래스 카드, Geist 타이포, violet 팔레트)를 그대로 재사용.

이후 메뉴 라벨을 "작동 방식"→"전체 이용 가이드", "작성 화면"→"이용 가이드"→최종 "사업계획서 작성"으로 두 차례 조정했고, 배포된 사이트를 직접 확인하는 과정에서 `/pricing` 페이지가 완성돼 있는데도 상단 메뉴 어디에도 링크가 없어 URL을 직접 입력해야만 갈 수 있었다는 것도 발견해 "가격" 링크를 추가했다.

---

### 7. Pro/Business 실제 가격 반영 + 베타 안내 페이지

**커밋:** `c1365e8`

`TBD`로 남아있던 요금제 페이지 가격을 Pro 9,900원/월, Business 55,000원/월로 확정했다. 세 플랜의 CTA 버튼("무료로 시작하기"/"Pro로 시작하기"/"문의하기")은 전부 신규 `/beta` 페이지로 연결 — 지금은 베타 기간이라 Pro·Business 유료 기능까지 전부 무료로 열려있다는 것을 전문적인 톤으로 안내한다. 무료 기능 목록에 Free 플랜의 제한 항목(로드맵 3회 등)까지 같이 넣었더니 "무제한"이랑 나란히 뜨는 게 앞뒤가 안 맞아서, Pro·Business 전용 기능만 모아서 보여주도록 정리했다.
...
### 4. 정보허브 기업마당·KOCCA 수집 — 존재하지 않던 API를 실제 크롤링으로 교체

**커밋:** `10fe7d2`

배포된 사이트에서 "정부지원사업 소식/스타트업 뉴스 제목과 링크가 안 맞는다"는 지적을 받고 조사하다가, 기업마당·KOCCA용 `fetch_*` 함수가 애초에 **존재하지 않는 API 엔드포인트**를 부르고 있었다는 걸 발견했다 — 이 두 기관은 K-Startup과 달리 공식 오픈API가 없다. 오픈소스 조사 도구 [ir-search](https://github.com/djfksjd/ir-search)(MIT)가 세 기관 모두 실제 라이브 호출로 검증해둔 필드명·엔드포인트를 참고해 기업마당·KOCCA는 공개 목록 페이지 크롤링으로 전면 재작성하고, 라이브로 직접 재검증(기업마당 15건, KOCCA 9건, 개별 공고 URL까지 정상 확인). K-Startup 응답 필드명도 검증된 snake_case 후보로 교체하고, API가 URL을 안 줄 때를 대비해 `pbanc_sn`으로 상세페이지 URL을 직접 조립하도록 추가 — 이게 없으면 모든 공고가 K-Startup 홈으로만 연결되는 문제가 있었다.
